...
type CoordinatorResponse = { id: number; code: string; inspector: number; rackAssigned: number; rackCompleted: number; status: string; sessionCode?: string; sessionLocation?: string };
function mapCoordinator(item: CoordinatorResponse): Coordinator { return { id: item.id, code: item.code, inspector: item.inspector, rackAssigned: item.rackAssigned, rackFinished: item.rackCompleted, status: item.status as CoordinatorStatus, sessionCode: item.sessionCode, sessionLocation: item.sessionLocation }; }
